...
Common spreads: **3/2/1** (specialist) or **2/2/2** (generalist). A 3 means misses are rare. A 1 means one die and a prayer.
...
Two tracks. Two currencies. You need to understand these before anything else makes sense.
...
That's how every roll works. The GM states what's at risk. You decide whether to roll, change approach, or back off.
...
| **1-3**     | Consequence. Things go sideways.                        |
...
Disadvantages impose **Snags (-1d).** Boons and Snags cancel 1-for-1. Only the net result applies. Minimum pool: **0d** (roll 2d6, take the lowest).

*Mae has Sharp 2 and her background applies (+1d). She's rolling 3d6. Gets a 6, 3, 2. Highest die: 6. Full success. The discrepancy is real. Two different names on the same intake record and nobody flagged it in nine years.*

### Zero Dice

*Niv at a Free Marches border checkpoint. Sharp 3, but it's dark (Snag) and the guard just flagged her papers (Snag). Net pool: 1d6.* One more Snag and she'd be at **0d**: roll 2d6, take the lowest. The worst odds in the game. *Niv has been there before.*
...
> **The Roll:** \[stat]d6, read highest. 6 = full, 4-5 = partial + cost, 1-3 = consequence. Boon = +1d, Snag = -1d, cancel 1:1. Min 0d = 2d6 take lowest. GM states stakes before you roll.
...
You see the dice. For each die showing 4+:

* **Sacrifice a 4-5** for a **standard** gambit.

* **Sacrifice a 6** for a **strong** gambit. Weapon dice (d8, d10, d12) can roll higher than 6, and those count too.

One die per gambit. You can use the same gambit more than once if you have the dice for it.

Only one die with no Boons or Drain? Damage OR gambit. Pick one.

### Combat Gambits

Seven verbs. Every player has access to these on attack and defense rolls. On stakes rolls, they're available as a menu alongside open-ended stakes gambits.
...
| **Setup** | **Bank the die** for yourself or an ally. Add to any roll after rolling. One banked die max per person. Gone at end of combat. | Bank for yourself **and** an ally. Same value, two dice. |
...
*A CSS contractor catches Hakim crossing open ground. 6 damage. Hakim is wearing a padded coat with a minor ward stitched into the lining (Armor 1). His Guard is at 4.*
...
Real injuries. Three levels, two slots each.

| Level | Name         | Effect                          | Overflow |
| ----- | ------------ | ------------------------------- | -------- |
| 1     | **Hurt**     | Snag when the injury interferes | 1-2      |
| 2     | **Wounded**  | Snag on all rolls               | 3-4      |
| 3     | **Critical** | Must spend Drain to act at all  | 5+       |

Each Harm is **named.** Player and GM agree on the injury from the fiction: grazed arm, cracked ribs, gut shot. The name determines when the Hurt Snag applies and how recovery works.

Both slots full at a level? New Harm bumps up. Both Critical slots full and you take more? You're done.
...
**Hurt:** Clears when the scene ends. A bandage kit clears it mid-scene (1 Act).

**Wounded:** Healing clock, 4 segments. Treatment rolls tick it (full = 2 ticks, partial = 1). Needs medical attention. Gear gives a Boon.

**Critical:** Healing clock, 6 segments (drops to Wounded when full). Needs training: a relevant Edge, Background, or a thaumatech trauma station.
...
When an effect forces you to resist, roll the specified stat. "Nerve save" means roll Nerve. Same result table: 6 = resist fully, 4-5 = reduced effect, 1-3 = full effect.

The triggering effect defines the consequences, not the GM. "Nerve save or Stunned" means Stunned is the 1-3 result. On a partial, it hits at reduced intensity: one action instead of a full turn, half distance, a Snag on your next roll only.

### PC Conditions

**Stunned.** Can't act. No Move, no Act, no reaction. Defense at 0d. Clears end of your next turn.

**Shaken.** Rattled, panicked, reeling. Snag on all rolls. Clears at end of scene (or earlier through ally help).

**Prone.** On the ground. Standing costs your Move. Melee attacks against you get a Boon. Ranged attacks against you get a Snag. Your melee attacks get a Snag.

> **Saves:** Roll specified stat. 6 = resist, 4-5 = partial, 1-3 = full effect. Boons/Snags apply normally. **Conditions:** Stunned (can't act, 0d defense), Shaken (Snag all, clears end of scene), Prone (stand costs Move).

***

## Your Gear

*Niv doesn't carry thaumatech. Her clicker reads 0.00. She carries two knives (d6 each, Subtle), three passports, and a reputation for arriving on time. Concealable. No questions at checkpoints that would ask plenty about a contact plate.*
...
*Luka, simple use. The siphon tap does what it's rated for. He rolls.* 6 = free. 4-5 = 1 Drain. 1-3 = 1 Drain + complication.

*Luka, stretching. The tap reads through a wall it wasn't built for.* 6 = 1 Drain. 4-5 = 2 Drain. 1-3 = 2 Drain + device at risk.

*Dr. Osei, way past spec. The stabilization rig suppresses a leyline breach it was never built for.* 6 = 2 Drain. 4-5 = 3 Drain + device at risk. 1-3 = 3 Drain + device breaks.

### Device at Risk

Device is strained. **Snag on all rolls using this device** until you spend an Act to fix it. If a strained device gets "at risk" again before you fix it, it breaks.
...
*Warehouse. Three hostiles. Hakim sees them through the loading dock window before they see him.*
...
Gambits apply. No setup if ambushed.

### Rounds

1. **Players act** in any order.
2. **Enemies act.**
3. **Environment.** Clocks tick, fires spread, structures shift.

### Your Turn

**Move** and an **Act**, either order. Skip either.

**Move:** One range band (Close, Near, Far) or reposition within a band.

**Act:** Attack, use a device, interact with the environment, or **Assist** (give an ally a Boon on their next roll).

### Three Lanes

**Hurting someone?** Combat roll. Weapon die + Boons + Drain, deal damage. Gambits from the seven-verb list.

**Changing the battlefield?** That's what gambits are for. Sacrifice a die from your combat roll.

**Something else entirely?** Stakes roll. Hack a terminal, barricade a door, talk someone down. GM sets stakes, you roll the relevant stat.
...
2. Declare Drain (+1d6) if spending. Must be before the roll.
3. Roll: weapon die + Boon d6s + Drain d6.
4. Sacrifice dice showing 4+ for gambits if you want them.
...
* **Sharp:** dodge, duck, dive behind something
...
**Cover:** Partial +1 Armor, Full +2 Armor. Ranged only.
...
*The false-bottom filing cabinet. Mae has a name. A positive who came in for registration nine years ago and never came out the other side.*
...
| **1-3** | Dead end, wrong conclusion, or something worse.          |
...
> **Work a Lead:** Need something concrete. [stat] roll. 6 = answer + follow-up, 4-5 = answer + cost, 1-3 = dead end or worse. **Group Checks:** Weakest Link (worst roll counts, cover for allies) or Group Effort (clock, everyone contributes).

***

## Quick Reference

**Stats:** Grit (body), Sharp (senses), Nerve (will). Rated 1-4. Sum = 6.
