...
1. **Attacks auto-hit.** No attack roll. Roll weapon dice for damage. Gambits are the tactical layer.
...
5. **Every attack is a choice.** Straight damage or sacrifice some for a Gambit effect. Offense vs. control trade-off on every swing.
...
- Example A: A fighter with Grit 3 uses a crowbar (1d6, Brutal). They sacrifice 4 damage for a Trip Gambit (standard). Enemy is Prone. Next attacker at Close gets a Boon.
- Example B: A runner with Sharp 3 fires from behind full cover (+2 Armor). They choose straight damage because the Danger Clock is at 5 and they need the kill before reinforcements arrive.
- Example C: A zero flanks a distracted guard (Boon from positioning). They attack with a stun baton (1d6) and spend Push for +1d on the Gambit check. No magic needed, no special abilities. Pure tactics.
