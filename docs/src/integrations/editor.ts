          ...
          configureServer(server: ViteDevServer) {
            ...
            server.middlewares.use(async (req, res, next) => {
              ...
              try {
                ...
                const { collection, entry, content } = JSON.parse(body);
                ...
                // Mark both paths so handleHotUpdate suppresses the reload
                recentlyWritten.add(realFile);
                recentlyWritten.add(symlink);
                // Safety: clear after 5s in case HMR never fires (e.g. file didn't actually change)
                setTimeout(() => {
                  recentlyWritten.delete(realFile);
                  recentlyWritten.delete(symlink);
                }, 5000);

                await writeFile(realFile, frontmatter + content, 'utf-8');

                console.log(`[editor] Saved ${collection}/${entry} → ${realFile}`);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ ok: true, path: realFile }));
              } catch (e: unknown) {
